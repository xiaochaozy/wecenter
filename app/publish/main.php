...
class main extends AWS_CONTROLLER
{
	public function get_access_rule()
	{
		...
		$rule_action['actions'] = array('index','pay','check');
		return $rule_action;
	}
	...
	public function check_action(){//开始进行付费选择页面 ，然后进行支付
		if ($_GET['qid']){
			$qid=intval($_GET['qid']);
			TPL::assign('qid', $qid);
		}
		TPL::output('publish/check');
		
	}
	public function pay_action(){//生成订单页面，跳转去支付
		$qid=intval($_GET['qid']);
		$zfid=intval($_GET['zfid']);
		TPL::output('publish/pay');
