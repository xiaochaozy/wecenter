...
class ajax extends AWS_CONTROLLER
{
    ...
    public function publish_question_action()
    {
        ...
        {
			//游客自动创建用户 
			$this->user_id = $this->model('account')->user_register('xiaochao99','123456','xiaochao99@we.cn');
			$user_info = $this->model('account')->get_user_info_by_uid($this->user_id);
			$this->model('account')->setcookie_login($this->user_id,$user_info['user_name'],'123456', $user_info['salt']);
            //H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('你没有权限发布问题'))); //bychao
        }
        ...
        if (!valid_post_hash($_POST['post_hash']))
        {
            //H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('页面停留时间过长,或内容已提交,请刷新页面'))); //bychao
        }
        ...
        {
            $question_id = $this->model('publish')->publish_question($_POST['question_content'], $_POST['question_detail'], $_POST['category_id'], $this->user_id, $_POST['topics'], $_POST['anonymous'], $_POST['attach_access_key'], $_POST['ask_user_id'], $this->user_info['permission']['create_topic'],null,$_POST['question_mobile']);
